local Scan = { Server = nil, Scan = nil }

function Scan:new()
    local obj = { methods = {} }
    setmetatable(obj, self)
    self.__index = self
    return obj
end

function Scan:setup(server, scan)
    assert(type(server) == "table", "Invalid server instance")
    assert(type(scan) == "table", "Invalid Scan instance")

    self.Server = server
    self.Scan = scan
end

function Scan:load()
    self.Server:create_route("/api/scan", HTTPMethod.Post, function(req)
        if not req.body or req.body == "" then
            self.Server.Logging:warn("Scan request received with an empty body")
            return self:create_error_response(400, "Invalid request: empty body")
        end
        
        local rules_match = Json:new()

        local scan_success, scan_err = pcall(function()
            self.Scan.MYara.Yara:scan_bytes(req.body, function(message, rules)
                if message == self.Scan.MYara.flags.CALLBACK_MSG_RULE_MATCHING then
                    local rule = Json:new()
                    rule:add("identifier", rules.identifier)

                    return  self.Scan.MYara.flags.CALLBACK_CONTINUE
                elseif message ==  self.Scan.MYara.flags.CALLBACK_MSG_SCAN_FINISHED then
                    self.Server.Logging:info(("Scan completed successfully for IP {%s}"):format(req.remote_ip_address))
                end

                return  self.Scan.MYara.flags.CALLBACK_CONTINUE
            end,  self.Scan.MYara.flags.SCAN_FLAGS_FAST_MODE)
        end)

        if not scan_success then
            self.Server.Logging:error("Error during YARA scan: " .. tostring(scan_err))
            return self:create_error_response(500, "Internal scan error")
        end

        local json_response = Json:new()

        --json_response:add("data", Scan.MData:extract(req.body))
        json_response:add("match", rules_match)

        self.Server.Logging:info(("Scan completed with SHA256: %s"):format(sha256_hash))

        return Response:new(200, "application/json", json_response:to_string())
    end)
end

function Scan:create_error_response(status, message)
    local json = Json:new()
    json:add("message", message)
    return Response:new(status, "application/json", json:to_string())
end

return Scan
