rule IDATDropper {
    meta:
        ...
        malpedia_family = "win.hijackloader"
    strings:
        $dialer = "dialer.pdb" ascii
        $BthUdTask = "BthUdTask.pdb" ascii
        $dvdplay = "dvdplay.pdb" ascii
        $script_start = "<script>" ascii
        $decode_from_charcode = "String.fromCharCode" ascii
        $script_end = "</script>" ascii
    condition:
        uint16(0) == 0x5a4d and (any of ($dialer, $BthUdTask, $dvdplay)) and (all of ($script_start, $decode_from_charcode, $script_end))
}