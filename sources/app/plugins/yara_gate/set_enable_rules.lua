local set_enable_rules = { server = nil, yara_manager = nil }

function set_enable_rules:new()
    local obj = { methods = {} }
...
function set_enable_rules:setup(server, yara_manager)
...
function set_enable_rules:load()
    local json = Json:new()
    self.server:create_route("/enable/rules", function(req)
        ...
        self.yara_manager.yara:rules_foreach(function(rules)
            if rules and rules.identifier == rule then
                rule_found = true
                self.yara_manager.yara:rule_enable(rules)
            end
        end)
...
function set_enable_rules:create_error_response(status, message)
    local json = Json:new()
    json:add("message", message)
    return Response:new(status, "application/json", json:tostring())
end

return set_enable_rules