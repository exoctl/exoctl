Web.new(_engine.server, "/status", function(req)
    ...
    local bridge_analysis = Json:new():add("scan_queue_size", _analysis.scan_queue_size)
    local bridge = Json:new():add("analysis", bridge_analysis)

    engine_fields:add("database", engine_database)
    engine_fields:add("server", engine_server)

    engine_fields:add("bridge", bridge)
