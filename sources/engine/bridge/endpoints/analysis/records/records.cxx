        ...
        analysis.map_.add_route("/records/update", [&]() {
            ...
            analysis.web_records_update_->setup(
                ...
                [&](const crow::request &req) -> const crow::response {
                    ...
                    if (req.body.empty()) {
                        return crow::response{
                            server::gateway::responses::BadRequests().code(),
                            "application/json",
                            server::gateway::responses::BadRequests()
                                .add_field("message", "Empty request body")
                                .tojson()
                                .tostring()};
                    }
                    ...
                    if (anal.sha256.empty()) {
                        return crow::response{
                            server::gateway::responses::NotFound().code(),
                            "application/json",
                            server::gateway::responses::NotFound()
                                .add_field(
                                    "message",
                                    fmt::format(
                                        "Record with sha256 '{}' not found",
                                        sha256.value()))
                                .tojson()
                                .tostring()};
                    }
                        ...
                        if (auto family_name =
                            ...
                            if (!family_name->empty()) {
                                ...
                                if (family.id != 0) {
                                    ...
                                    analysis.server_->log->warn(fmt::format(
                    ...
                    if (auto tags_array =
                        ...
                        for (const auto &tag_json : *tags_array) {
                            ...
                            if (tag.id != 0) {
                                ...
                                analysis.server_->log->warn(fmt::format(
                                    "Tag '{}' not found, skipping", *tag_name));
                            }
                        }
                    }
                    ...
                    return crow::response{
                        server::gateway::responses::Accepted().code(),
                        "application/json",
                        server::gateway::responses::Accepted()
                            .add_field("sha256", anal.sha256)
                            .tojson()
                            .tostring()};
                });
        });
