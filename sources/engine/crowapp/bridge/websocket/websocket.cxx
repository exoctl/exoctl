    ...
    namespace bridge
    {
        WebSocket::WebSocket(CrowApp &p_crow,
            ...
            : m_crow(p_crow), m_url(p_url), m_context(p_crow.crow_get_config()),
              ...
              m_on_accept(on_accept), m_on_open(on_open), m_on_close(on_close)
        {
            ...
            m_crow.crow_get_app()
                ...
                .onopen([this](crow::websocket::connection &p_conn) {
                    WebSocket::websocket_def_open_connection(&p_conn);
                    if (m_on_open)
                        m_on_open(m_context, p_conn);
                })
                .onclose([this](crow::websocket::connection &p_conn,
                                const std::string &p_reason,
                                uint16_t p_code) {
                    WebSocket::websocket_def_close_connection(&p_conn,
                                                              p_reason);
                    if (m_on_close)
                ...
                .onmessage([this](crow::websocket::connection &p_conn,
                                  ...
                                  bool p_is_binary) {
                    WebSocket::websocket_def_message_connection(&p_conn,
                                                                p_data);
                    if (m_on_message)
            ...
            crow::websocket::connection *p_conn, const std::string &p_reason)
        {
            ...
            m_context.context_erase(p_conn);
        ...
        {
            std::lock_guard<std::mutex> _(m_mtx);
            m_context.context_add(p_conn);
            m_context.context_broadcast(p_conn, "{\"status\": \"ready\"}");

            LOG(m_crow.crow_get_log(),
                info,
                "Connection opened {} from IP: {}",
                m_url,
                m_context.context_get_remote_ip(p_conn));
        }

        bool WebSocket::websocket_def_accept_connection(
            const crow::request *p_req)
        {
            std::lock_guard<std::mutex> _(m_mtx);
            if (m_context.context_check_whitelist(p_req))
                return true;
