...
namespace crowapp
{
    ...
    void Bridge::scan_clamav()
    {
        m_scan_clamav = std::make_unique<focades::analysis::scan::Clamav>(
            m_crow.get_config());

        LOG(m_crow.get_log(), info, "Loading rules database clamav ...");
    ...
    void Bridge::scan_yara()
    {
        m_scan_yara = std::make_unique<focades::analysis::scan::Yara>(
            m_crow.get_config());

        TRY_BEGIN()
        LOG(m_crow.get_log(), info, "Loading rules yara ...");
        ...
        CATCH(security::yara::exception::LoadRules, {
            LOG(m_crow.get_log(), error, "{}", e.what());
    ...
    void Bridge::load()
    {
        LOG(m_crow.get_log(), info, "Initializing Routes ... ");
        TRY_BEGIN()
        GET_ROUTE(metadata);
        ...
        CATCH(std::exception, {
            LOG(m_crow.get_log(), warn, "{}", e.what());
