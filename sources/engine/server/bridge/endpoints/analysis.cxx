...
namespace engine::server::bridge::endpoints
{
    ...
    void Analysis::scan()
    {
        m_map.add_route("/scan", [&]() {
            m_socket_scan = std::make_unique<engine::server::gateway::WebSocket>();
            m_socket_scan->setup(
                m_server,
                BASE_ANALYSIS "/scan",
                UINT64_MAX,
    ...
    void Analysis::scan_av_clamav()
    {
        m_map.add_route("/scan/av/clamav", [&]() {
            m_socket_scan_av_clamav = std::make_unique<engine::server::gateway::WebSocket>();
    ...
    void Analysis::scan_yara()
    {
        m_map.add_route("/scan/yara/fast", [&]() {
            m_socket_scan_yara = std::make_unique<engine::server::gateway::WebSocket>();
    ...
    {
        LOG(m_server.get_log(), info, "Preparing gateway analysis routes ...");
